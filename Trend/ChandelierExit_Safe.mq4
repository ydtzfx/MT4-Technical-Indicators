...
double longStop[],shortStop[],trendLine[],buySignal[],sellSignal[];
...
int start() {
   ...
   for(int i=limit;i>=2;i--) {
      if(shortStop[i+1]!=EMPTY_VALUE&&longStop[i]!=EMPTY_VALUE)buySignal[i]=iLow(_Symbol,_Period,i)-10*Point;
      if(longStop[i+1]!=EMPTY_VALUE&&shortStop[i]!=EMPTY_VALUE)sellSignal[i]=iHigh(_Symbol,_Period,i)+10*Point;
   }
   if(Bars>0){longStop[0]=longStop[1];shortStop[0]=shortStop[1];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
