...
double hma[],buySignal[],sellSignal[];

int init() {
   SetIndexStyle(0,DRAW_LINE,STYLE_SOLID,InpWidth,InpColor);SetIndexBuffer(0,hma);SetIndexLabel(0,"HMA");
...
int start() {
   ...
   for(int i=limit;i>=2;i--){
      ...
      if(c1<=hma[i+1]&&c>hma[i])buySignal[i]=iLow(_Symbol,_Period,i)-5*Point;
      if(c1>=hma[i+1]&&c<hma[i])sellSignal[i]=iHigh(_Symbol,_Period,i)+5*Point;
   }
   if(Bars>0){hma[0]=hma[1];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
