...
int start() {
   ...
   for(i=Bars-InpPeriod-1;i>=1;i--){
      if(i>=Bars-InpPeriod-2){double s=0;for(int j=0;j<InpPeriod;j++)s+=tr[i+j];atr[i]=s/InpPeriod;s=0;for(j=0;j<InpPeriod;j++)s+=pdm[i+j];pdiRaw[i]=s/InpPeriod;s=0;for(j=0;j<InpPeriod;j++)s+=mdm[i+j];mdiRaw[i]=s/InpPeriod;}
   ...
   for(i=Bars-InpPeriod*2-1;i>=1;i--){
      if(i>=Bars-InpPeriod*2-2){s=0;for(j=0;j<InpPeriod;j++)s+=dx[i+j];adx[i]=s/InpPeriod;}
