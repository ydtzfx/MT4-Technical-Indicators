...
double ma[20][],buySignal[],sellSignal[];
...
int start() {
   ...
   if(InpShowSignals) for(int i=limit;i>=2;i--) {
      // 最快线(MA2)穿越最慢线(MA40)=趋势转换
      if(ma[0][i+1]<=ma[19][i+1]&&ma[0][i]>ma[19][i])buySignal[i]=iLow(_Symbol,_Period,i)-20*Point;
      if(ma[0][i+1]>=ma[19][i+1]&&ma[0][i]<ma[19][i])sellSignal[i]=iHigh(_Symbol,_Period,i)+20*Point;
   }
   if(Bars>0){for(int m=0;m<20;m++)ma[m][0]=ma[m][1];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
