...
double ama[],buySignal[],sellSignal[];

int init() {
   SetIndexStyle(0,DRAW_LINE,STYLE_SOLID,2,clrDodgerBlue);SetIndexBuffer(0,ama);SetIndexLabel(0,"KAMA");
...
int start() {
   ...
   for(int i=limit;i>=1;i--){ama[i]=ama[i]>0?ama[i]:iClose(_Symbol,_Period,i);buySignal[i]=EMPTY_VALUE;sellSignal[i]=EMPTY_VALUE;}
   ...
   if(Bars>0){ama[0]=ama[1];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
