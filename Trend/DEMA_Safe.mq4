...
double dema[],buySignal[],sellSignal[];
...
int start() {
   ...
   for(int i=limit;i>=1;i--){dema[i]=2*e1[i]-e2[i];buySignal[i]=EMPTY_VALUE;sellSignal[i]=EMPTY_VALUE;}
   for(int i=limit;i>=2;i--){
      double c=iClose(_Symbol,_Period,i),c1=iClose(_Symbol,_Period,i+1);
      if(c1<=dema[i+1]&&c>dema[i])buySignal[i]=iLow(_Symbol,_Period,i)-5*Point;
      if(c1>=dema[i+1]&&c<dema[i])sellSignal[i]=iHigh(_Symbol,_Period,i)+5*Point;
   }
   if(Bars>0){dema[0]=2*e1[0]-e2[0];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
