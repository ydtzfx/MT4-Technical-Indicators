...
double tema[],buySignal[],sellSignal[];

int init() {
   SetIndexStyle(0,DRAW_LINE,STYLE_SOLID,2,InpColor);SetIndexBuffer(0,tema);SetIndexLabel(0,"TEMA");
...
int start() {
   ...
   for(int i=limit;i>=1;i--){tema[i]=3*e1[i]-3*e2[i]+e3[i];buySignal[i]=EMPTY_VALUE;sellSignal[i]=EMPTY_VALUE;}
   for(int i=limit;i>=2;i--){
      double c=iClose(_Symbol,_Period,i),c1=iClose(_Symbol,_Period,i+1);
      if(c1<=tema[i+1]&&c>tema[i])buySignal[i]=iLow(_Symbol,_Period,i)-5*Point;
      if(c1>=tema[i+1]&&c<tema[i])sellSignal[i]=iHigh(_Symbol,_Period,i)+5*Point;
   }
   if(Bars>0){tema[0]=3*e1[0]-3*e2[0]+e3[0];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
