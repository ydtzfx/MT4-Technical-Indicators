...
int init() {
   SetIndexStyle(0,DRAW_LINE,STYLE_SOLID,1,clrRoyalBlue);SetIndexBuffer(0,upper);SetIndexEmptyValue(0,EMPTY_VALUE);SetIndexLabel(0,"Upper");
   SetIndexStyle(1,DRAW_LINE,STYLE_SOLID,2,clrOrange);SetIndexBuffer(1,mid);SetIndexEmptyValue(1,EMPTY_VALUE);SetIndexLabel(1,"Mid");
   SetIndexStyle(2,DRAW_LINE,STYLE_SOLID,1,clrRoyalBlue);SetIndexBuffer(2,lower);SetIndexEmptyValue(2,EMPTY_VALUE);SetIndexLabel(2,"Lower");
...
int start() {
   ...
   for(int i=limit;i>=1;i--) {
      ...
      double trSum=0;for(j=0;j<InpATRPeriod;j++)trSum+=GetTrueRange(_Symbol,_Period,i+j);double atr=trSum/InpATRPeriod;
