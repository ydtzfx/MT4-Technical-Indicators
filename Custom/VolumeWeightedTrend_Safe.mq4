...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      // 信号线
      double s=0;for(j=0;j<InpSignal;j++)s+=vwt[i+j];signal[i]=s/InpSignal;

      // 置信度 = 近期成交量 / 均量（高量=趋势更可靠）
      double curV=0,avgV=0;for(j=0;j<5;j++)curV+=iVolume(_Symbol,_Period,i+j);curV/=5;
      for(j=0;j<20;j++)avgV+=iVolume(_Symbol,_Period,i+j);avgV/=20;
