...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      for(j=2;j<InpLookback;j++){double h=iHigh(_Symbol,_Period,i+j),l=iLow(_Symbol,_Period,i+j);if(h>hh)hh=h;if(l<ll)ll=l;}
      ...
      if(breakUp>InpBreakThreshold||breakDn>InpBreakThreshold){
         ...
         // ADX趋势强度
         double adxS=0;for(j=0;j<14;j++)adxS+=GetTrueRange(_Symbol,_Period,i+j);adxS/=14;
