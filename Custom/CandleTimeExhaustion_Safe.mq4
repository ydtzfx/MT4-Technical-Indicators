...
int start(){int cb=IndicatorCounted();if(cb<0)cb=0;int limit=Bars-cb;if(limit>Bars-2)limit=Bars-200;if(limit<0)limit=0;
   ...
   for(i=limit;i>=InpMaxBars;i--){
      ...
      for(j=1;j<InpMaxBars;j++){double h=iHigh(_Symbol,_Period,i+j),l=iLow(_Symbol,_Period,i+j);if(h>rngHi)rngHi=h;if(l<rngLo)rngLo=l;if((rngHi-rngLo)<atr*InpRangeThreshold)consBars++;else break;}
