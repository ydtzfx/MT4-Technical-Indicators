...
double zhongShuHi[],zhongShuLo[],buySignal[],sellSignal[];
...
int start(){int cb=IndicatorCounted();if(cb<0)cb=0;int limit=Bars-cb;if(limit>Bars-2)limit=Bars-300;if(limit<0)limit=0;
   ...
   for(int i=limit;i>=0;i--){zhongShuHi[i]=EMPTY_VALUE;zhongShuLo[i]=EMPTY_VALUE;buySignal[i]=EMPTY_VALUE;sellSignal[i]=EMPTY_VALUE;}
   // 找摆动点，每3段重叠=中枢
   ...
   for(int s=3;s<segCnt-1;s+=2){double zsH=MathMin(segHi[s],MathMin(segHi[s-1],segHi[s-2]));double zsL=MathMax(segLo[s],MathMax(segLo[s-1],segLo[s-2]));if(zsH>zsL&&segBars[s]<=limit){zhongShuHi[segBars[s]]=zsH;zhongShuLo[segBars[s]]=zsL;double c=iClose(_Symbol,_Period,segBars[s]);if(c>zsH)buySignal[segBars[s]]=zsL-5*Point;else if(c<zsL)sellSignal[segBars[s]]=zsH+5*Point;}}
   return(0);}
