...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      for(j=0;j<InpLookback/2;j++)volSum1+=iVolume(_Symbol,_Period,i+j);
      for(j=InpLookback/2;j<InpLookback;j++)volSum2+=iVolume(_Symbol,_Period,i+j);
      ...
      for(j=0;j<3;j++)atr3+=GetTrueRange(_Symbol,_Period,i+j);
      for(j=0;j<InpLookback;j++)atr10+=GetTrueRange(_Symbol,_Period,i+j);
