...
int start() {
   ...
   for(i=limit;i>=3;i--){
      ...
      double fast=0,slow=0;for(j=79;j>=0;j--){if(j==79){fast=p[j];slow=p[j];}else{double aF=2.0/(InpFastMA+1);fast=p[j]*aF+fast*(1-aF);double aS=2.0/(InpSlowMA+1);slow=p[j]*aS+slow*(1-aS);}}

      // ADX
      double trS=0,ps=0,ms=0;for(j=0;j<14;j++){int s=i+j;double hi=iHigh(_Symbol,_Period,s),lo=iLow(_Symbol,_Period,s),pc=iClose(_Symbol,_Period,s+1);trS+=MathMax(hi-lo,MathMax(MathAbs(hi-pc),MathAbs(lo-pc)));double up=hi-iHigh(_Symbol,_Period,s+1),dn=iLow(_Symbol,_Period,s+1)-lo;if(up>dn&&up>0)ps+=up;if(dn>up&&dn>0)ms+=dn;}
      double adx=SafeDivide(100*MathAbs(ps-ms),ps+ms,0);

      // 成交量
      double v=iVolume(_Symbol,_Period,i),vAvg=0;for(j=0;j<20;j++)vAvg+=iVolume(_Symbol,_Period,i+j);vAvg/=20;
