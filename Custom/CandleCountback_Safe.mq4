...
int start(){int cb=IndicatorCounted();if(cb<0)cb=0;int limit=Bars-cb;if(limit>Bars-2)limit=Bars-100;if(limit<0)limit=0;
   for(int i=limit;i>=1;i--){
      ...
      for(j=lowBar-1;j>=1;j--){if(iClose(_Symbol,_Period,j)>iClose(_Symbol,_Period,j+2))buyCount++;else break;if(buyCount>=InpBase)break;}
      // 卖出计数：找到最高点后，依次比较收盘是否低于前第2根
      int sellCount=0;double highest=iHigh(_Symbol,_Period,i+1);int highBar=i+1;
      for(j=2;j<20;j++){if(iHigh(_Symbol,_Period,i+j)>highest){highest=iHigh(_Symbol,_Period,i+j);highBar=i+j;}}
      for(j=highBar-1;j>=1;j--){if(iClose(_Symbol,_Period,j)<iClose(_Symbol,_Period,j+2))sellCount++;else break;if(sellCount>=InpBase)break;}
