...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      for(j=0;j<5;j++){s=trendBar+j;double h=iHigh(_Symbol,InpTrendTF,s),l=iLow(_Symbol,InpTrendTF,s);if(h>stH)stH=h;if(l<stL)stL=l;}
      double stoch=SafeDivide(100*(iClose(_Symbol,InpTrendTF,trendBar)-stL),stH-stL,50);

      // 小周期：RSI
      double aG=0,aL=0;for(j=0;j<InpRSIPeriod;j++){double ch=iClose(_Symbol,_Period,i+j)-iClose(_Symbol,_Period,i+j+1);if(ch>0)aG+=ch;else aL-=ch;}
