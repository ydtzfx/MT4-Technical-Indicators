...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      double fE=0,sE=0;for(int jjj=99;j>=0;j--){if(j==99){fE=p[j];sE=p[j];}else{double aF=2.0/13,aS=2.0/27;fE=p[j]*aF+fE*(1-aF);sE=p[j]*aS+sE*(1-aS);}}
      double macd=fE-sE;if(macd>0)score+=macd>0.0001?2:1;else score-=macd<-0.0001?2:1;

      // 3. Stochastic (<20=+2, >80=-2)
      double stH=iHigh(_Symbol,_Period,i),stL=iLow(_Symbol,_Period,i);for(int jjjj=0;j<5;j++){double h=iHigh(_Symbol,_Period,i+j),l=iLow(_Symbol,_Period,i+j);if(h>stH)stH=h;if(l<stL)stL=l;}
      double stoch=SafeDivide(100*(iClose(_Symbol,_Period,i)-stL),stH-stL,50);score+=(stoch<20?2:stoch<30?1:stoch>80?-2:stoch>70?-1:0);

      // 4. ADX (>30且+DI>-DI=+1)
      double tS=0,ps=0,ms=0;for(int jjjjj=0;j<14;j++){int s=i+j;double hi=iHigh(_Symbol,_Period,s),lo=iLow(_Symbol,_Period,s),pc=iClose(_Symbol,_Period,s+1);tS+=MathMax(hi-lo,MathMax(MathAbs(hi-pc),MathAbs(lo-pc)));double up=hi-iHigh(_Symbol,_Period,s+1),dn=iLow(_Symbol,_Period,s+1)-lo;if(up>dn&&up>0)ps+=up;if(dn>up&&dn>0)ms+=dn;}
      double adx=SafeDivide(100*MathAbs(ps-ms),ps+ms,0);if(adx>25)score+=(ps>ms?1:-1);

      // 5. MA排列 (MA10>MA20>MA50=+2)
      double ma10=0,ma20=0,ma50=0;for(int jjjjjj=0;j<10;j++)ma10+=iClose(_Symbol,_Period,i+j);ma10/=10;
      for(int jjjjjjj=0;j<20;j++)ma20+=iClose(_Symbol,_Period,i+j);ma20/=20;
      for(jjjjjjj=0;j<50;j++)ma50+=iClose(_Symbol,_Period,i+j);ma50/=50;
      score+=(ma10>ma20&&ma20>ma50?2:ma10>ma20?1:ma10<ma20&&ma20<ma50?-2:ma10<ma20?-1:0);

      // 6. BB位置 (<下轨=+1, >上轨=-1)
      double sum=0;for(jjjjjjj=0;j<20;j++)sum+=iClose(_Symbol,_Period,i+j);double sma=sum/20;
      double sd=0;for(jjjjjjj=0;j<20;j++){double d=iClose(_Symbol,_Period,i+j)-sma;sd+=d*d;}sd=MathSqrt(sd/20);
