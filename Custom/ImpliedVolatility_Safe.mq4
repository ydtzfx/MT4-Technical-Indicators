...
int start(){int cb=IndicatorCounted();if(cb<0)cb=0;int limit=Bars-cb;if(limit>Bars-2)limit=Bars-100;if(limit<0)limit=0;
   for(int i=limit;i>=1;i--){
      ...
      double mean=sum/InpIVPeriod;double var=0;for(j=0;j<InpIVPeriod;j++){double d=ret[j]-mean;var+=d*d;}
   ...
   for(i=limit;i>=1;i--){double s=0;for(j=0;j<InpSignalPeriod;j++)s+=iv[i+j];ivMA[i]=s/InpSignalPeriod;}
   for(i=limit;i>=2;i--){
      // IV从极高位回落 = 恐慌消退 → 可能反弹
