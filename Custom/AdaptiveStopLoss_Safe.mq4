...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      // 因子1：近期波动率 vs 长期波动率
      double atr5=0,atr50=0;for(j=0;j<5;j++)atr5+=GetTrueRange(_Symbol,_Period,i+j);
      for(j=0;j<50&&(i+j<Bars);j++)atr50+=GetTrueRange(_Symbol,_Period,i+j);atr5/=5;atr50/=50;
      double volAdj=SafeDivide(atr5,atr50,1);volAdj=MathMax(0.7,MathMin(1.5,volAdj));

      // 因子2：ADX趋势强度
      double trS=0,pdS=0,mdS=0;
      for(j=0;j<14;j++){int s=i+j;double h=iHigh(_Symbol,_Period,s),l=iLow(_Symbol,_Period,s),pc=iClose(_Symbol,_Period,s+1);trS+=MathMax(h-l,MathMax(MathAbs(h-pc),MathAbs(l-pc)));double up=h-iHigh(_Symbol,_Period,s+1),dn=iLow(_Symbol,_Period,s+1)-l;if(up>dn&&up>0)pdS+=up;if(dn>up&&dn>0)mdS+=dn;}
      ...
      for(j=2;j<20;j++){h=iHigh(_Symbol,_Period,i+j);l=iLow(_Symbol,_Period,i+j);if(h>hh)hh=h;if(l<ll)ll=l;}
