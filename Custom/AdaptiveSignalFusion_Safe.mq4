...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      for(j=99;j>=0;j--){if(j==99){eF=p[j];eS=p[j];}else{eF=p[j]*aF+eF*(1-aF);eS=p[j]*aS+eS*(1-aS);}}
      ...
      for(j=0;j<InpStochK;j++){double h=iHigh(_Symbol,_Period,i+j),l=iLow(_Symbol,_Period,i+j);if(h>hh)hh=h;if(l<ll)ll=l;}
      double stoch=SafeDivide(100*(iClose(_Symbol,_Period,i)-ll),hh-ll,50);
      double stochSignal=(stoch-50)*2;

      // === 波动率权重计算（基于ATR相对于其均值的偏离）===
      double atr5=0,atr20=0;for(j=0;j<5;j++)atr5+=GetTrueRange(_Symbol,_Period,i+j);
      for(j=0;j<20;j++)atr20+=GetTrueRange(_Symbol,_Period,i+j);atr5/=5;atr20/=20;
      ...
      for(j=4;j>=0;j--)sE=fusionLine[i+j]*aSig+sE*(1-aSig);
      signalLine[i]=sE;
