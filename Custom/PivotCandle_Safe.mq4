...
int start(){int cb=IndicatorCounted();if(cb<0)cb=0;int limit=Bars-cb;if(limit>Bars-2)limit=Bars-200;if(limit<0)limit=0;
   ...
   for(i=limit;i>=InpSwingBars;i--){
      ...
      for(j=1;j<=InpSwingBars;j++){if(i+j<Bars&&iLow(_Symbol,_Period,i+j)<=l)isLow=false;if(i-j>=0&&iLow(_Symbol,_Period,i-j)<=l)isLow=false;}
      if(isLow){pivotLow[i]=l-5*Point;
         for(k=i-1;k>=1;k--){if(iHigh(_Symbol,_Period,k)>iHigh(_Symbol,_Period,i)){buySignal[k]=iLow(_Symbol,_Period,k)-10*Point;break;}}}
   }
   return(0);}
