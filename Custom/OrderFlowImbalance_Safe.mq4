...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      double eSm=delta[i+InpSmooth];for(j=InpSmooth-1;j>=0;j--)eSm=delta[i+j]*aS+eSm*(1-aS);
      ofi[i]=eSm;signal[i]=0;buySignal[i]=EMPTY_VALUE;sellSignal[i]=EMPTY_VALUE;
   }
   // 信号线
   for(i=limit;i>=1;i--){s=0;for(j=0;j<InpSmooth;j++)s+=ofi[i+j];signal[i]=s/InpSmooth;}
   for(i=limit;i>=2;i--){
