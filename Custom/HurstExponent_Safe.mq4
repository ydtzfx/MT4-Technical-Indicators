   ...
   for(int i=limit;i>=1;i--){
      ...
      for(int s=0;s<4;s++){
         ...
         for(int start=0;start<InpPeriod-subN;start+=subN/2){
            double mean=0;for(j=0;j<subN;j++)mean+=returns[start+j];mean/=subN;
            double dev[];ArrayResize(dev,subN);double cumDev=0;
            double maxDD=0,minDD=999;for(j=0;j<subN;j++){cumDev+=returns[start+j]-mean;dev[j]=cumDev;if(cumDev>maxDD)maxDD=cumDev;if(cumDev<minDD)minDD=cumDev;}
            double range=maxDD-minDD;
            double std=0;for(j=0;j<subN;j++)std+=(returns[start+j]-mean)*(returns[start+j]-mean);std=MathSqrt(std/subN);
