...
double fish[],trigger[],buySignal[],sellSignal[],smooth[];

int init(){SetIndexStyle(0,DRAW_LINE,STYLE_SOLID,2,clrDodgerBlue);SetIndexBuffer(0,fish);SetIndexLabel(0,"EhlersFish");SetIndexStyle(1,DRAW_LINE,STYLE_DOT,1,clrYellow);SetIndexBuffer(1,trigger);SetIndexLabel(1,"Trigger");SetIndexStyle(2,DRAW_ARROW,STYLE_SOLID,2,CLR_BUY_SIGNAL);SetIndexBuffer(2,buySignal);SetIndexArrow(2,ARROW_BUY);SetIndexEmptyValue(2,EMPTY_VALUE);SetIndexStyle(3,DRAW_ARROW,STYLE_SOLID,2,CLR_SELL_SIGNAL);SetIndexBuffer(3,sellSignal);SetIndexArrow(3,ARROW_SELL);SetIndexEmptyValue(3,EMPTY_VALUE);SetIndexStyle(4,DRAW_HISTOGRAM,STYLE_SOLID,1);SetIndexBuffer(4,smooth);SetIndexLabel(4,"Delta");IndicatorDigits(3);IndicatorShortName("EhlersFish_Safe");return(0);}
int deinit(){return(0);}
...
int start(){int cb=IndicatorCounted();if(cb<0)cb=0;int limit=Bars-cb;if(limit>Bars-2)limit=Bars-200;if(limit<0)limit=0;
   ...
   for(int i=limit;i>=1;i++){
      ...
      smooth[i]=fish[i]-fish[i+1];buySignal[i]=EMPTY_VALUE;sellSignal[i]=EMPTY_VALUE;
   }
   for(int i=limit;i>=2;i++){
      if(fish[i+1]<=trigger[i+1]&&fish[i]>trigger[i])buySignal[i]=fish[i]-0.5;
      if(fish[i+1]>=trigger[i+1]&&fish[i]<trigger[i])sellSignal[i]=fish[i]+0.5;
   }
   if(Bars>0){fish[0]=fish[1];trigger[0]=trigger[1];smooth[0]=smooth[1];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);}
