...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      ...
      // MACD信号
      double prices[100];for(j=0;j<100;j++)prices[j]=iClose(_Symbol,_Period,i+j);
      double aF=2.0/(InpMACDFast+1),aS=2.0/(InpMACDSlow+1);
      double eF=prices[99],eS=prices[99];
      for(j=98;j>=0;j--){eF=prices[j]*aF+eF*(1-aF);eS=prices[j]*aS+eS*(1-aS);}
      double macdLine=eF-eS;
      if(macdLine>0)totalScore+=(macdLine>0.0001?30:15);else totalScore-=(macdLine<-0.0001?30:15);

      // 均线排列
      double ma10=0,ma20=0,ma60=0;for(j=0;j<10;j++)ma10+=iClose(_Symbol,_Period,i+j);ma10/=10;
      for(j=0;j<20;j++)ma20+=iClose(_Symbol,_Period,i+j);ma20/=20;
      for(j=0;j<60;j++)ma60+=iClose(_Symbol,_Period,i+j);ma60/=60;
      if(ma10>ma20&&ma20>ma60)totalScore+=30;else if(ma10<ma20&&ma20<ma60)totalScore-=30;

      // 趋势强度（连续上涨天数）
      int upDays=0;for(j=0;j<5;j++)if(iClose(_Symbol,_Period,i+j)>iClose(_Symbol,_Period,i+j+1))upDays++;
