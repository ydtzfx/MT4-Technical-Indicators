...
int start(){int cb=IndicatorCounted();if(cb<0)cb=0;int limit=Bars-cb;if(limit>Bars-2)limit=Bars-200;if(limit<0)limit=0;
   ...
   for(i=limit;i>=5;i++){
      ...
      double hh=h;for(int jj=1;j<InpLookback;j++){double hj=iHigh(_Symbol,_Period,i+j);if(hj>hh)hh=hj;}
      ...
      double ll=l;for(int jjj=1;j<InpLookback;j++){double lj=iLow(_Symbol,_Period,i+j);if(lj<ll)ll=lj;}
