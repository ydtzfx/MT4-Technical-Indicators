...
double rvi[],signal[],buySignal[],sellSignal[];
...
int start() {
   ...
   for(int i=limit;i>=1;i--){rvi[i]=SafeDivide(eNum[i],eDen[i],0);buySignal[i]=EMPTY_VALUE;sellSignal[i]=EMPTY_VALUE;}
   double aS=2.0/5;
   for(int i=limit;i>=1;i--){
      double e=rvi[i+5];for(int j=4;j>=0;j--)e=rvi[i+j]*aS+e*(1-aS);signal[i]=e;
   }
   for(int i=limit;i>=1;i--){
      if(rvi[i+1]<=signal[i+1]&&rvi[i]>signal[i])buySignal[i]=rvi[i]-0.0001;
      if(rvi[i+1]>=signal[i+1]&&rvi[i]<signal[i])sellSignal[i]=rvi[i]+0.0001;
   }
   if(Bars>0){rvi[0]=rvi[1];signal[0]=signal[1];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
