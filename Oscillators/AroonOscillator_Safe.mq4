...
double aoBuffer[],buySignal[],sellSignal[];
...
int start() {
   ...
   for(int i=limit;i>=1;i--) {
      if(aoBuffer[i+1]<0&&aoBuffer[i]>0)buySignal[i]=-5;
      if(aoBuffer[i+1]>0&&aoBuffer[i]<0)sellSignal[i]=5;
   }
   if(Bars>0){aoBuffer[0]=aoBuffer[1];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
