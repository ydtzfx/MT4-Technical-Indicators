...
int start() {
   ...
   for(int i=hist;i>=1;i--) {
      ...
      double emaF=0;for(j=hist-1;j>=hist-InpMACDFast;j--)emaF+=prices[j];emaF/=InpMACDFast;
      double aF=2.0/(InpMACDFast+1);for(j=hist-InpMACDFast-1;j>=0;j--)emaF=prices[j]*aF+emaF*(1-aF);
      double emaS=0;for(j=hist-1;j>=hist-InpMACDSlow;j--)emaS+=prices[j];emaS/=InpMACDSlow;
      double aS=2.0/(InpMACDSlow+1);for(j=hist-InpMACDSlow-1;j>=0;j--)emaS=prices[j]*aS+emaS*(1-aS);
      macd[i]=emaF-emaS;
   }
   // Stochastic of MACD
   for(i=hist;i>=1;i--) {
      double mn=macd[i],mx=macd[i];for(j=0;j<InpCycle;j++){if(macd[i+j]<mn)mn=macd[i+j];if(macd[i+j]>mx)mx=macd[i+j];}
      double rng=mx-mn;stoch[i]=MathAbs(rng)>0?100*(macd[i]-mn)/rng:50;
   }
   // EMA of Stochastic
   double aC=2.0/(InpCycle+1);
   for(i=hist;i>=1;i--) {
      double e=stoch[i+InpCycle];for(j=InpCycle-1;j>=0;j--)e=stoch[i+j]*aC+e*(1-aC);
