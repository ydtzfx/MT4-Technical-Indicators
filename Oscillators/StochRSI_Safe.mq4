...
int start() {
   ...
   for(i=limit;i>=1;i--){
      double mn=rsi[i],mx=rsi[i];for(j=0;j<InpStochPeriod;j++){if(rsi[i+j]<mn)mn=rsi[i+j];if(rsi[i+j]>mx)mx=rsi[i+j];}
   ...
   for(i=limit;i>=1;i--){double s=0;for(j=0;j<InpSmooth;j++)s+=srsi[i+j];signal[i]=s/InpSmooth;}
   for(i=limit;i>=1;i--){
