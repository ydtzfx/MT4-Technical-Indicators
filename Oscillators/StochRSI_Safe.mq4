...
double srsi[],signal[],buySignal[],sellSignal[];

int init() {
   SetIndexStyle(0,DRAW_LINE,STYLE_SOLID,1,clrDodgerBlue);SetIndexBuffer(0,srsi);SetIndexLabel(0,"StochRSI");
...
int start() {
   ...
   for(int i=limit;i>=1;i--){
      if(srsi[i+1]<=0.2&&srsi[i]>0.2)buySignal[i]=0.15;
      if(srsi[i+1]>=0.8&&srsi[i]<0.8)sellSignal[i]=0.85;
      if(srsi[i+1]<=signal[i+1]&&srsi[i]>signal[i]&&srsi[i]<0.5)buySignal[i]=0.15;
      if(srsi[i+1]>=signal[i+1]&&srsi[i]<signal[i]&&srsi[i]>0.5)sellSignal[i]=0.85;
   }
   if(Bars>0){srsi[0]=srsi[1];signal[0]=signal[1];buySignal[0]=sellSignal[0]=EMPTY_VALUE;}
   return(0);
}
